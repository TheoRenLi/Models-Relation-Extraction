...
from model_and_config.capsuleLayersStuff import L2Regularizer, MarginLoss
...
class Config(object):
    def __init__(self):
        ...
        self.num_caps = int(self.hidden_size / 16)
        self.routing_iter = 3
        self.pos_size = 30
        self.tag_size = 30
        self.max_epoch = 25
    ...
    def set_train_model(self, model):
        ...
        #     if 'bias' in name or 'digitCaps' in name:
        #         print(name)
        #         bias.append(param)
        ...
        if self.opt_method == 'Adadelta':
            ...
            self.optimizer = optim.Adadelta([
                ...
                {'params': weight, 'weight_decay': self.weight_decay}
        ...
        self.loss = MarginLoss().cuda()
        # self.l2_reg = L2Regularizer(model=self.trainModel, lambda_reg=self.weight_decay, name_reg=name_weight)
    ...
    def dataloader(self, is_type='train'):
        if is_type == 'train':
            ...
            valid_iter = Data.DataLoader(valid_dataset, batch_size=self.batch_size, shuffle=False)
            return valid_iter
        else:
    ...
    def train_one_step(self, batch, label):
        ...
        # loss = self.l2_reg.regularize_all_param(loss)
    ...
    def train(self):
        ...
        for epoch in range(self.max_epoch):
            ...
            self.testModel.eval()
            macro = self.test_one_epoch()
            if macro['f1-score'] > best_F1:
                best_F1 = macro['f1-score']
            # valid_loss.append(valid_loss_epoch)

        # writer.close()
        self.loss_plot(train_loss)
        return np.around(best_F1, 4)
    ...
    def one_epoch_loss(self):
        ...
        with torch.no_grad():
            for batch, label in valid_iter:
                ...
                batch_pos2 = Variable(batch[:, self.max_length * 3:self.max_length * 4].cuda())
                batch_label = Variable(label.cuda())
                output = self.testModel.test(batch_word, batch_tag, batch_pos1, batch_pos2)
                loss = self.loss(output, batch_label)
                # loss = self.l2_reg.regularize_all_param(loss)
        ...
        # print("valid loss: ", np.mean(data_loss))
